  ...
  setUpCard(location, section, id, adjustedSection) {
    ...
    for (var prop in location) {
      if (checkAgainstCategories.indexOf(prop) < 0) {
        ...
        basicProps = basicProps.concat('<p> <span class="card-cat">' + '<strong>' + adjusted + '</strong>' + ': </span>' + location[prop] + '</p>')
      }
    }
    ...
    if (location[adjustedSection + "Plus"]) {
      var plus = '<p> <span class="card-cat"> +: </span>' + location[adjustedSection + "Plus"] + '</p>';
    } else {
      var plus = "";
    }
    var content =
    ...
    '<p><a rel="nofollow" class="DARTlink" href="https://maps.google.com/?q=1200' + location.streetAddress + ' ' + location.city + ', ' + location.state + ' ' + location.zip  + '">' + location.streetAddress + ' ' + location.city + ', ' + location.state + ' ' + location.zip + '</a></p>' +
    basicProps +
    other +
    plus +
    '<div>';
