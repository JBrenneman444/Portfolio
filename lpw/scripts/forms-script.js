...
//CheckWidth is to make it look good on all browsers
//Go to JQUERY to learn how to change anything with a $ sign
...
//USE as starting point for CHECKING USER vs EMAIL etc
function checkDuplicates () {
  if ($('#venueAddress').val().length > 0 && $('#venueCity').val().length > 0 && $('#venueState').val().length > 0) {
    if ($('#formSelector').val() === "Update") {
    } else {
